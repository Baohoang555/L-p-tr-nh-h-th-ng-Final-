    ...
    public class ApplicationCoordinator
    {
        ...
        private async Task CalculateInitialKpisAsync()
        {
            ...
            if (aggregates == null || aggregates.Count == 0)
            {
                ...
                // Tạo report trống để tránh lỗi NullReference sau này
                _currentReport = new KpiReport
                {
                    ReportId = Guid.NewGuid().ToString(),
                    ExportedDate = DateTime.Now,
                    Details = new List<ProductKpi>(),
                    SystemWide = new SystemWideKpi()
                };
            }
            else
            {
                _currentReport = await _kpiCalculator.GenerateReportAsync(aggregates);

                if (_currentReport?.Details != null)
                {
            ...
            // 4. Hiển thị kết quả lên màn hình Console
            if (_currentReport != null)
            {
                _formatter.DisplayKpiReport(_currentReport);
            }

            _logger.LogInfo($"KPIs calculated for {aggregates?.Count ?? 0} unique products");
        }
        ...
        private async Task AutoReportLoopAsync()
        {
            while (!_cts.Token.IsCancellationRequested)
            {
                try
                {
                    ...
                    // 1. Lấy dữ liệu từ Service của nhóm
                    var aggregates = _kpiUpdater.GetAllAggregates();

                    if (aggregates != null && aggregates.Count > 0)
                    {
                        // 2. Gọi hàm Async
                        var report = await _kpiCalculator.GenerateReportAsync(aggregates);

                        // 3. Xuất file JSON qua Service
                        string folderName = "Reports";
                        string fileName = $"KPI_Report_{DateTime.Now:yyyyMMdd_HHmm}.json";
                        ...
                        await _reportExporter.ExportReportAsync(filePath, report);

                        _logger.LogInfo($"Đã xuất báo cáo thành công: {fileName}");
                    }
                }
                catch (TaskCanceledException) { break; }
                catch (Exception ex)
                {
                    _logger.LogError("Lỗi tự động xuất báo cáo", ex);
                }
            }
        }

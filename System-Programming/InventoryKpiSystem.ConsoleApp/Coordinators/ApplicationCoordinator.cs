...
namespace InventoryKpiSystem.ConsoleApp.Coordinators
{
    public class ApplicationCoordinator
    {
        ...
        private List<Product> _products = new();   // ← THÊM: lưu catalog để tính TotalSkus
        ...
        public ApplicationCoordinator(AppConfig config)
        {
            ...
            _kpiUpdater = new IncrementalKpiUpdater();  // ← FIX: truyền list rỗng, sẽ update sau khi load catalog
        ...
        private async Task LoadHistoricalDataAsync()
        {
            try
            {
                // ← THÊM: đọc product catalog trước để có TotalSkus đúng
                _products = await _xeroImporter.ReadProductFileAsync(_config.ProductFilePath);
                _logger.LogInfo($"Loaded {_products.Count} products from catalog");
        ...
        private async Task CalculateInitialKpisAsync()
        {
            ...
            {
                // ← FIX: truyền _products để TotalSkus đếm từ catalog (~1,399)
                _currentReport = _kpiCalculator.GenerateReport(aggregates, _products);
